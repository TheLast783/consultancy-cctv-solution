...
def process_pending_item():
    ...
    c = conn.cursor()
    c.execute("SELECT id, image_path FROM events WHERE vlm_verdict = 'PENDING' ORDER BY id ASC LIMIT 1")
    row = c.fetchone()
    conn.close()
    ...
    try:
        ...
        print(f"VLM Timeout Warning: Ollama did not respond within 90s. Unblocking queue for {img_path}...", flush=True)
        conn = sqlite3.connect(DB_PATH, timeout=20.0)
        c = conn.cursor()
        ...
        conn = sqlite3.connect(DB_PATH, timeout=20.0)
        c = conn.cursor()
        c.execute("UPDATE events SET vlm_verdict = 'ERROR' WHERE id = ?", (event_id,))
        conn.commit()
        conn.close()
...
if __name__ == "__main__":
    ...
    while True:
        processed = process_pending_item()
        if not processed:
            time.sleep(2)
